...
func TestMempoolAtmTxsAppGossipHandling(t *testing.T) {
	...
	txGossipedLock.Lock()
	assert.Equal(2, txGossiped, "tx should have been gossiped")
	...
	assert.NoError(vm.AppGossip(context.Background(), nodeID, msgBytes))
	txGossipedLock.Lock()
	assert.Equal(2, txGossiped, "tx should have been gossiped twice")
	txGossipedLock.Unlock()
	...
	assert.False(txRequested, "tx should not have been requested")
	txGossipedLock.Lock()
	assert.Equal(2, txGossiped, "tx should not have been gossiped")
	txGossipedLock.Unlock()
